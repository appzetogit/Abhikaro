...
export default function OTP() {
  ...
  const handleVerify = async (otpValue = null) => {
    ...
    try {
      const { phone, email, isSignUp } = authData
      ...
      // Always persist user auth in localStorage so session survives app restarts,
      // independent of the 'Remember me' checkbox
      setUserAuthData("user", accessToken, user, { persistent: true })
  ...
  const handleResend = async () => {
    ...
    try {
      const { phone, email, isSignUp } = authData
      await authAPI.sendOTP(phone, isSignUp ? "register" : "login", email)
      startResendTimer()
      setOtp(["", "", "", "", "", ""])
      inputRefs.current[0]?.focus()
    } catch (err) {
  ...
  return (
    <AnimatedPage className="min-h-screen bg-white dark:bg-[#0a0a0a] flex flex-col">
      {/* Header */}
      <div className="relative flex items-center justify-center pt-8 pb-4 px-4 md:pt-10 md:pb-6 md:px-6 lg:px-8 border-b border-gray-200 dark:border-gray-800">
        <button
