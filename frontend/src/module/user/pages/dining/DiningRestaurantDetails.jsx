...
export default function DiningRestaurantDetails() {
    ...
    const handleShareClick = async () => {
        ...
        const shareName = (restaurant.diningConfig?.basicDetails?.name && restaurant.diningConfig.basicDetails.name.trim()) || restaurant.name || "Dining"
        const shareTitle = `${shareName} - Tastizo`
        const shareText = `Book your table at ${shareName} on Tastizo.`
    ...
    // Helper values (dynamic from API) — prefer dining config so owner-set name/address show here
    ...
    const restaurantPhone = ownerPhone || restaurant.phone || restaurant.primaryContactNumber || ""
                ...
                <div className="absolute bottom-0 left-0 w-full p-5 text-white">
                    ...
                    {(ownerName || ownerPhone) && (
                        <p className="text-xs text-white/80 mb-2">
                            {ownerName && <span>Contact: {ownerName}</span>}
                            {ownerName && ownerPhone && " • "}
            ...
            <div className="p-4 min-h-[300px]">
                ...
                {activeTab === "Photos" && (
                    <div className="space-y-3">
                        <div className="grid grid-cols-2 gap-3">
                            {coverImage && (
                                <img src={coverImage} alt="" className="w-full aspect-square object-cover rounded-xl" />
                            )}
                            {Array.isArray(dc?.gallery) && dc.gallery
                                .filter((img) => img?.url)
                                .map((img, i) => (
                                    <img key={i} src={img.url} alt="" className="w-full aspect-square object-cover rounded-xl" />
                                ))}
                            {!coverImage && (!dc?.gallery || dc.gallery.length === 0) && restaurant.profileImage?.url && (
