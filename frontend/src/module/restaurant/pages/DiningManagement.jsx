    ...
    const fetch = async () => {
      try {
        ...
        if (configRes.data?.success && configRes.data?.data?.diningConfig) {
          ...
          const ownerName = configRes.data?.data?.ownerName || ""
          setConfig(c)
          if (c.adminControls) {
            setAdminControls({
            ...
            <div className="flex items-center gap-3">
              ...
              {!adminControls.isEnabledByAdmin && adminControls.requestStatus !== "pending" && (
                <Button
                  ...
                  disabled={saving}
                  className="shrink-0 border-amber-500/50 text-amber-700 hover:bg-amber-50"
                >
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-4">
        ...
        >
          <div className="grid gap-4">
            <label className="block text-sm font-medium text-slate-700">Display name (owner / restaurant name)</label>
            <Input value={form.basicDetails.name} onChange={(e) => setForm((p) => ({ ...p, basicDetails: { ...p.basicDetails, name: e.target.value } }))} placeholder="e.g. owner or restaurant name" className="max-w-md" />
