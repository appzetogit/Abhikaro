...
export const RestaurantSocketContext = createContext(null);

export const RestaurantSocketProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // NOTE: forcing polling prevents noisy "WebSocket ... failed" errors on networks
    // where websocket upgrade is blocked or unreliable.
    socketRef.current = io(socketUrl, {
      path: '/socket.io/',
      // Prefer WebSockets for instant restaurant notifications, fallback to polling
  ...
  // Fast status polling as a bulletproof fallback for PM2 multi-process cluster isolation
  // (Ensures the restaurant status stays in sync even if socket broadcasts are isolated/missed)
  useEffect(() => {
    ...
    const pollStatus = async () => {
      try {
        ...
        if (restaurant && typeof restaurant.isAcceptingOrders === 'boolean') {
          ...
          // Only trigger state updates/events if the status actually changed
          const prevStatus = localStorage.getItem('restaurant_online_status');
          if (prevStatus === null || JSON.parse(prevStatus) !== isOnline) {
            console.log('🔄 Restaurant status synced via polling:', isOnline);
            localStorage.setItem('restaurant_online_status', JSON.stringify(isOnline));
            window.dispatchEvent(new CustomEvent('restaurantStatusChanged', { 
              detail: { isOnline } 
            }));
    ...
    const statusPollInterval = setInterval(pollStatus, 30000);
    
    // Also run once immediately after restaurantId is fetched
    pollStatus();
