...
export default function HotelTermsGuard({ children }) {
  ...
  const checkStatus = useCallback(async () => {
    ...
    try {
      const res = await hotelAPI.getTermsAcceptanceStatus()
      const data = res?.data?.data
      if (data) {
        setRequiresAcceptance(Boolean(data.requiresAcceptance))
        if (data.terms) {
          setTerms({
            title: data.terms.title || "Hotel Terms and Conditions",
      ...
      console.error("Failed to check hotel terms status:", err)
    } finally {
      setChecking(false)
    }
  }, [])
  ...
  useEffect(() => {
    ...
    window.addEventListener("hotel_terms_updated", handleTermsUpdated)
    return () => {
      window.removeEventListener("hotel_terms_updated", handleTermsUpdated)
    }
  }, [checkStatus])
      ...
      <AnimatePresence>
        {requiresAcceptance && (
          ...
          >
            ...
            <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-700 text-white px-4 sm:px-6 md:px-8 py-4 sm:py-5 shadow-md flex items-center gap-3.5 sm:gap-4 shrink-0 sticky top-0 z-30">
              ...
              <div className="space-y-0.5 sm:space-y-1">
                ...
                <p className="text-xs sm:text-sm text-blue-100 leading-snug">
                  Please review and accept the updated Terms & Conditions to access your {companyName} Hotel Portal.
                </p>
              </div>
            </div>
            ...
            <div className="flex-1 w-full max-w-4xl mx-auto px-4 sm:px-6 md:px-10 py-6 sm:py-8 space-y-6">
              <div className="border-b border-gray-200 pb-4 space-y-1">
                <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">
                  {terms.title || "Hotel Terms and Conditions"}
                </h2>
              ...
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: terms.content || "<p>Hotel Terms & Conditions content</p>",
                }}
              />

              {/* Action Box at the very end / last of terms */}
              <div className="mt-10 mb-8 p-5 sm:p-7 bg-blue-50/70 border-2 border-blue-200 rounded-2xl space-y-4 shadow-sm">
                ...
                <label className="flex items-start gap-3 cursor-pointer select-none group">
                  <input
                    ...
                    className="mt-0.5 w-5 h-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500 cursor-pointer transition-colors"
                  />
                  <span className="text-xs sm:text-sm font-medium text-gray-800 group-hover:text-gray-900 leading-snug">
                    I have read, understood, and agree to the {companyName} Hotel Terms & Conditions.
                  </span>
                </label>
                ...
                <button
                  ...
                  className={`w-full py-3.5 px-6 rounded-xl font-semibold text-sm sm:text-base text-white shadow-md flex items-center justify-center gap-2 transition-all ${
                    agreed && !accepting
                      ? "bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 active:scale-[0.99] cursor-pointer"
