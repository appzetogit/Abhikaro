...
export default function DeliveryOrderHistoryTable({
  loading = false,
  orders = [],
  onViewOrder,
  onPrintOrder,
  ...
  if (!loading && (!orders || orders.length === 0)) {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 py-16 text-center text-slate-600">
        No orders found for the selected delivery boy and filters.
      </div>
    )
  }
        ...
        <table className="w-full min-w-[1150px]">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <th className="px-4 py-3 text-right text-[10px] font-bold text-slate-700 uppercase tracking-wider">Wallet Credit</th>
              <th className="px-4 py-3 text-right text-[10px] font-bold text-slate-700 uppercase tracking-wider">Diff</th>
          ...
          <tbody className="bg-white divide-y divide-slate-100">
            {paginated.map((o, idx) => {
              ...
              const walletC = o.walletCreditAmount == null ? null : Number(o.walletCreditAmount ?? 0)
              const diff = walletC == null ? null : walletC - deliveryE
              return (
                <tr key={o.id || o._id || o.orderId}>
                  ...
                  <td className="px-4 py-3 text-sm">
                    <div className="flex flex-col">
                      ...
                      {o.paymentStatus && (
                        <span className={`text-xs ${o.paymentStatus === "Paid" ? "text-emerald-600" : "text-amber-600"}`}>
                          {o.paymentStatus}
                        </span>
                      )}
                    </div>
                  ...
                  <td className="px-4 py-3 text-sm text-right text-slate-800">{currency(deliveryE)}</td>
                  <td className="px-4 py-3 text-sm text-right font-semibold text-emerald-700">
                    {walletC == null ? "—" : currency(walletC)}
                  </td>
                  <td className="px-4 py-3 text-sm text-right">
                    {diff == null ? (
                      <span className="text-slate-500">—</span>
                    ) : (
                      <span className={diff === 0 ? "text-slate-700" : diff > 0 ? "text-emerald-700 font-semibold" : "text-rose-700 font-semibold"}>
                        {currency(diff)}
                      </span>
                    )}
                  </td>
