...
const CACHE_KEY = "admin_order_detect_delivery"

function OrderDetectDeliverySkeleton() {
  return (
    <div className="p-4 lg:p-6 bg-slate-50 min-h-screen animate-pulse">
      {/* Topbar Skeleton */}
...
export default function OrderDetectDelivery() {
  ...
  // Instant SWR: direct cache or derived from admin_orders_all (0ms mount)
  const resolveInitialOrders = () => {
    const directCache = getOrdersCache(CACHE_KEY)
    ...
    if (Array.isArray(allCache) && allCache.length > 0) {
      try {
        const transformed = allCache.map((order, index) => transformOrder(order, index))
        return transformed
      } catch (e) {
        return []
      }
  ...
  const fetchOrders = async (isSilent = false) => {
    try {
      ...
      if (response.data?.success && response.data?.data?.orders) {
        ...
        setOrdersCache(CACHE_KEY, transformedOrders)
      } else {
        if (!isSilent) {
  ...
  useEffect(() => {
    const hasCache = initialOrders.length > 0
  ...
  if (isLoading && orders.length === 0) {
    return <OrderDetectDeliverySkeleton />
  }
