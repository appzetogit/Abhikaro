...
// Export hotel API helper functions
...
export const hotelAPI = {
  ...
  sendOTP: (phone, purpose = "login") => {
    return apiClient.post(API_ENDPOINTS.HOTEL.AUTH.SEND_OTP, {
      phone,
      purpose,
    });
  },
  ...
  verifyOTP: (
    ...
    if (hotelName != null) payload.hotelName = hotelName;
    if (email != null) payload.email = email;
  ...
  refreshToken: () => {
    return apiClient.post(API_ENDPOINTS.HOTEL.AUTH.REFRESH_TOKEN);
  },

  logout: () => {
    return apiClient.post(API_ENDPOINTS.HOTEL.AUTH.LOGOUT);
  },
  ...
  // Get hotel profile
  getProfile: () => {
    return apiClient.get(API_ENDPOINTS.HOTEL.PROFILE);
  },
  ...
  // Get request stats
  getRequestStats: () => {
    return apiClient.get(API_ENDPOINTS.HOTEL.REQUEST_STATS);
  },
...
export const adminAPI = {
  ...
  // Get hotels
  getHotels: (params = {}) => {
    return apiClient.get(API_ENDPOINTS.ADMIN.HOTELS, { params });
  },
  ...
  // Create hotel
  createHotel: (data) => {
    return apiClient.post(API_ENDPOINTS.ADMIN.HOTELS, data);
  },

  // Get hotel by ID
  getHotelById: (id) => {
    return apiClient.get(API_ENDPOINTS.ADMIN.HOTEL_BY_ID.replace(":id", id));
  },
  ...
  updateHotelCashCollected: (hotelId, cashCollected, options = {}) => {
    const amount = Number(cashCollected);
    const balanceAdjustment = options?.balanceAdjustment;
