...
async function run() {
  try {
    ...
    console.log(`Analyzing ${orders.length} delivered orders:`);
    let totalRestEarning = 0;
    ...
    for (const order of orders) {
      ...
      // ----------------------------------------------------
      // EXACT replica of orderController.js calculation
      // ----------------------------------------------------
      ...
      totalRestEarning += restaurantEarning;
      
      console.log(`Order: ${order.orderId} | QR: ${isHotelQrOrder} | Subtotal: ${subtotal} | Disc: ${discount} | RestEarning: ${restaurantEarning}`);
    }

    console.log(`\nRe-calculated total restaurant earnings: ₹${totalRestEarning.toFixed(2)}`);
