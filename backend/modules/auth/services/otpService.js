...
/**
 * Check if identifier (phone or email) is a test identifier
 * @param {string} phone - Phone number
 * @param {string} email - Email address
 * @returns {boolean} - True if either is a test identifier
 */
const isTestIdentifier = (phone, email) => {
  return (phone && isTestPhoneNumber(phone)) || (email && isTestEmail(email));
};
...
class OTPService {
  ...
  async generateAndSendOTP(phone = null, purpose = "login", email = null) {
    try {
      ...
      logger.info(
        `OTP generated and sent to ${identifier} (${identifierType})`,
        {
          [identifierType]: identifier,
          purpose,
          otpId: otpRecord._id,
  ...
  async verifyOTP(phone = null, otp, purpose = "login", email = null) {
    try {
      ...
      // DEBUG LOG
      logger.info(
        `OTP Verification Attempt: ${identifierType}=${identifier}, otp=${otp}, purpose=${purpose}`,
      );
      ...
      logger.info(
        `OTP verified successfully for ${identifier} (${identifierType})`,
        {
          [identifierType]: identifier,
          purpose,
        },
