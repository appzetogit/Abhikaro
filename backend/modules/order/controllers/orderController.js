...
export const createOrder = async (req, res) => {
  try {
    ...
    const order = new Order({
      ...
      // Add hotel reference if order came from hotel QR code
    ...
    if (normalizedPaymentMethod === 'pay_at_hotel') {
      ...
      try {
        ...
        logger.error('❌ Error creating Pay at Hotel payment record (continuing without blocking order):', {
          error: paymentError.message,
          stack: paymentError.stack
        });
      }

      // Mark order as confirmed so restaurant can prepare it (ensure payment.method is pay_at_hotel for notification)
      ...
      // Notify restaurant about new Pay at Hotel order via Socket.IO (non-blocking)
      try {
        const notifyRestaurantResult = await notifyRestaurantNewOrder(order, assignedRestaurantId, 'pay_at_hotel');
        logger.info('✅ Pay at Hotel order notification sent to restaurant', {
          orderId: order.orderId,
          restaurantId: assignedRestaurantId,
          hotelReference: hotelReference || 'N/A',
          notifyRestaurantResult
        ...
        logger.error('⚠️ Failed to notify restaurant about Pay at Hotel order (non-blocking):', {
      ...
      return res.json({
        success: true,
        message: 'Order placed successfully. Pay at Hotel.',
        data: {
