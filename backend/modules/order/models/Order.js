...
const orderSchema = new mongoose.Schema(
  {
    ...
    hotelReference: {
      type: String,
      default: null, // Hotel ID if order came from hotel QR code
      index: true,
    },
    hotelName: {
      type: String,
      default: null, // Hotel name for reference
    },
    hotelId: {
      type: mongoose.Schema.Types.ObjectId,
      ref: "Hotel",
    ...
    roomNumber: {
      ...
      validate: {
        validator: function (value) {
          // Room number is required only for pay_at_hotel payment method
          if (this.payment && this.payment.method === "pay_at_hotel") {
            return value && value.trim().length > 0;
          }
          return true; // Not required for other payment methods
        },
    ...
    address: {
      label: {
        type: String,
        enum: ["Home", "Office", "Other", "Live", "live"],
      },
      street: String,
    ...
    payment: {
      method: {
        type: String,
        enum: ["razorpay", "cash", "wallet", "upi", "card", "pay_at_hotel"],
        required: true,
      },
      status: {
        type: String,
