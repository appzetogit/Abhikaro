...
const orderSettlementSchema = new mongoose.Schema(
  {
    ...
    adminEarning: {
      ...
      hotelCommission: { type: Number, default: 0, min: 0 }, // Admin commission from hotel QR orders
      orderType: {
        type: String,
        enum: ["DIRECT", "QR"],
        default: "DIRECT",
      ...
      adminCommissionStatus: {
        type: String,
        enum: ["none", "pending_settlement", "received"],
        default: "none",
      },
    ...
    // Hotel Earnings (for orders from QR code scans)
    hotelEarning: {
      hotelId: {
        type: mongoose.Schema.Types.ObjectId,
        ref: "Hotel",
      ...
      hotelName: { type: String, sparse: true },
      commission: { type: Number, default: 0, min: 0 }, // Hotel commission percentage of order total
      commissionPercentage: { type: Number, default: 0, min: 0 },
      status: {
        type: String,
        enum: ["pending", "credited", "cancelled"],
        default: "pending",
...
const OrderSettlement = mongoose.model(
  "OrderSettlement",
