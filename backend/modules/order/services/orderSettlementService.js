...
export const calculateOrderSettlement = async (orderId) => {
  try {
    ...
    // Calculate hotel and admin commissions for QR code orders
    let hotelEarning = {
      hotelId: null,
      hotelName: null,
      commission: 0,
    ...
    let hotelCommissionAmount = 0;
    let adminCommissionFromHotel = 0;
    
    // If order came from hotel QR code scan, calculate hotel and admin commissions
    if (order.hotelReference) {
      let hotel = null;
      // Try to find hotel by hotelId or _id
      if (mongoose.Types.ObjectId.isValid(order.hotelReference) && order.hotelReference.length === 24) {
        hotel = await Hotel.findById(order.hotelReference).lean();
      }
      if (!hotel) {
        hotel = await Hotel.findOne({ 
          $or: [
            { hotelId: order.hotelReference },
            { _id: order.hotelReference }
          ]
        }).lean();
      }
      
      if (hotel) {
        // Calculate commissions on order total (₹100)
        const orderTotal = userPayment.total;
        const hotelCommissionPercent = hotel.commission || 0;
        const adminCommissionPercent = hotel.adminCommission || 0;
        
        // Hotel commission: percentage of order total
        hotelCommissionAmount = Math.round((orderTotal * hotelCommissionPercent / 100) * 100) / 100;
        
        // Admin commission from hotel: percentage of order total
        adminCommissionFromHotel = Math.round((orderTotal * adminCommissionPercent / 100) * 100) / 100;
        
        hotelEarning = {
          hotelId: hotel._id,
          hotelName: hotel.hotelName || order.hotelName || 'Unknown Hotel',
          commission: hotelCommissionAmount,
          commissionPercentage: hotelCommissionPercent,
          status: 'pending'
        };
      }
    }
    ...
    // Admin gets: Restaurant commission + Platform fee + Delivery fee + GST + Hotel admin commission
    ...
    const adminTotal = Math.round((adminCommission + adminPlatformFee + adminDeliveryFee + adminGST + adminCommissionFromHotel) * 100) / 100;
    ...
    const adminEarning = {
      ...
      hotelCommission: adminCommissionFromHotel, // Admin commission from hotel QR orders (₹20)
    ...
    const settlementData = {
      ...
      calculationSnapshot: {
        ...
        hotelCommission: hotelEarning.hotelId ? {
          ...
          adminCommissionPercentage: adminCommissionFromHotel > 0 ? ((adminCommissionFromHotel / userPayment.total) * 100).toFixed(2) : 0,
