...
export const sendMessage = async (req, res) => {
  try {
    ...
    const orderIdStr = order.orderId ? String(order.orderId).trim() : null;
    ...
    try {
      ...
      if (io) {
        ...
        io.to(receiverType === "user" ? `user:${receiverIdStr}` : `delivery:${receiverIdStr}`).emit("new-message", messageData);
      }
    } catch (emitErr) {
      console.error("[Chat] Socket emit error (message saved):", emitErr);
    }
